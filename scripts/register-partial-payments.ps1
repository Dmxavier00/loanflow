...
$paymentsDir = Join-Path $PSScriptRoot "..\artifacts\payment-batches"
New-Item -ItemType Directory -Path $paymentsDir -Force | Out-Null

if ([string]::IsNullOrWhiteSpace($BatchFile)) {
    $latest = Get-ChildItem (Join-Path $PSScriptRoot "..\artifacts\seed-batches\seed-batch-*.json") |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1
    if (-not $latest) {
        throw "Nenhum arquivo de lote encontrado em artifacts\\seed-batches."
    }
